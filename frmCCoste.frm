...
Begin VB.Form frmCCoste 
   ...
   Begin VB.TextBox Text1 
      ...
      MaxLength       =   30
      TabIndex        =   1
      Tag             =   "FDenominación C.C.|T|N|||cabccost|nomccost|||"
   ...
   Begin VB.TextBox Text1 
      ...
      MaxLength       =   4
      TabIndex        =   0
      Tag             =   "Codigo C.C.|T|N|||cabccost|codccost||S|"
      Text            =   "Text1"
      Top             =   900
      Width           =   1095
   End
...
Private WithEvents frmBB As frmBuscaGrid   'Este sera para buscar los datos
...
Private Sub Check1_Click()
    ...
    If Modo = 4 Then
        ...
        If Check1.Value = 1 Then
            SQL = DevuelveDesdeBD("linscost", "linccost", "subccost", Text1(0).Text, "T")
            If SQL <> "" Then
...
Private Sub BotonEliminar()
    ...
    If I = vbYes Then
        ...
        Cad = "Delete from cabccost where nomccost = '" & Data1.Recordset!codccost & "'"
        Conn.Execute Cad
        CargaGrid Data1.Recordset!codccost
...
'    Adodc1.password = vUsu.Passwd
    ...
    'Data1.RecordSource = "Select * from cabccost ORDER BY nomccost"
    Data1.Refresh
    'Cargamos el grid
    CargaGrid "-11"
...
Private Function MontaSQLCarga(vNumC As String) As String
    ...
    SQL = "SELECT linccost.codccost,linccost.linscost,linccost.subccost, cabccost.nomccost,linccost.porccost"
    SQL = SQL & " FROM linccost INNER JOIN cabccost ON linccost.subccost = cabccost.codccost"
    SQL = SQL & " WHERE (((linccost.codccost)='" & vNumC & "'))"
    SQL = SQL & " ORDER BY linccost.linscost;"
    MontaSQLCarga = SQL
End Function
...
Private Sub AnyadirLinea()
    ...
    If MaxValor = 0 Then
        MsgBox "El % total de los subcentros de coste ya es 100.", vbExclamation
        Exit Sub
    End If
...
Private Sub EliminarLineaFactura()
    ...
    If MsgBox(SQL, vbQuestion + vbYesNoCancel) = vbYes Then
        SQL = "Delete from linccost WHERE codccost ='" & Data1.Recordset!codccost
...
Private Function SugerirCodigoSiguiente() As Integer
    ...
    SQL = "Select Max(linscost) from linccost where codccost='" & Data1.Recordset!codccost & "'"
    Set RS = New ADODB.Recordset
...
If Index = 0 Then
   txtaux(0).Text = UCase(txtaux(0).Text)
    ...
    Cad = "idsubcos"
    SQL = DevuelveDesdeBD("nomccost", "cabccost", "codccost", txtaux(0).Text, "T", Cad)
    Sng = 0
    If SQL <> "" Then
...
Dim Sng As Single
    Set RS = New ADODB.Recordset
    SQL = "SELECT * FROM linccost where codccost='" & Data1.Recordset!codccost & "'"
...
On Error GoTo EInsertarModificarCCoste
    ...
    If ModificaLineas = 1 Then
        ...
        Else
        SQL = "UPDATE linccost Set subccost = '" & txtaux(0).Text
...
On Error GoTo EPorcentajeCorrecto
    ...
    SQL = "Select sum(porccost) from linccost"
...
Private Function CentroYaAsignado(Cod As String, SubCod As String) As Boolean
    ...
    SQL = "Select * from linccost where codccost = '" & Cod & "'"
...
On Error GoTo EBorrarCC
    ...
    SQL = DevuelveDesdeBD("codinmov", "sinmov", "codccost", Text1(0).Text, "T")
    If SQL <> "" Then
    ...
    SQL = DevuelveDesdeBD("codinmov", "sbasin", "codccost", Text1(0).Text, "T")
    If SQL <> "" Then
    ...
    'Cuenta bancaria
    SQL = DevuelveDesdeBD("codccost", "ctabancaria", "codccost", Text1(0).Text, "T")
    If SQL <> "" Then
