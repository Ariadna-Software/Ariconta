...
Begin VB.Form frmActualizar 
   ...
   Begin VB.Frame FrameListaContabilizar 
      ...
      Begin VB.Image imgCheck 
         ...
         Index           =   0
         Left            =   240
         Picture         =   "frmActualizar.frx":0156
   ...
   Begin VB.Frame FrameFacturas 
      ...
      Begin VB.Image Image3 
         Height          =   240
         Left            =   840
         Picture         =   "frmActualizar.frx":02A0
         Top             =   2760
         Width           =   240
      End
      ...
      Begin VB.Image Image2 
         ...
         Index           =   2
         Left            =   840
         Picture         =   "frmActualizar.frx":6AF2
         Top             =   720
         Width           =   240
      End
      Begin VB.Image Image2 
         ...
         Index           =   3
         Left            =   3000
         Picture         =   "frmActualizar.frx":6B7D
         Top             =   720
         Width           =   240
      End
...
On Error GoTo EComprobarFactura
    ...
    If OpcionActualizar = 10 Then
        ...
        SQL = SQL & " ,trefaccl as importetrefaccl,numasien, 0 extranje FROM cabfact"
    ...
    Else
        ...
        SQL = SQL & " ,trefacpr as importetrefaccl,numasien,extranje FROM cabfactprov"
    ...
    Else
        ...
        'Le sumamos los IVAS.
        'En clientes esta puesto un 0 para que sume siempre los IVAS. Cuando hagamos los
        'cambios en las intracomunitarias, habra que tocar el trozo de clientes
        If RT!extranje <> 3 And RT!extranje <> 1 Then
...
Dim Mc As Contadores
    ...
    If Me.OpcionActualizar = 7 Then
        ...
        SQL = SQL & " AND anofaccl= " & RecuperaValor(NUmSerie, 2)
        'Las lineas
        Donde = "Linea factura"
        Cuenta = "DELETE from linfact " & SQL
        Conn.Execute Cuenta
        'La factura
        Donde = "Cabecera factura"
        Cuenta = "DELETE from cabfact " & SQL
        Conn.Execute Cuenta
    ...
    Else
        ...
        '-------------------------------------------------------------
        SQL = " WHERE numregis = " & NumFac
        SQL = SQL & " AND anofacpr= " & NUmSerie
        'Las lineas
        Donde = "Linea factura"
        Cuenta = "DELETE from linfactprov " & SQL
        Conn.Execute Cuenta
        'La factura
        Donde = "Cabecera factura"
        Cuenta = "DELETE from cabfactprov " & SQL
        Conn.Execute Cuenta
        LEtra = "1"
    End If
...
Dim PrimeraContrapartida As String
    ...
    DocConcAmp = "'" & Format(NumFac, "00000") & "'," & vParam.concefcl & ",'"
    ...
    A_Donde = "IVA 1"
    Cad3 = "cuentarr"
    Cad2 = DevuelveDesdeBD("cuentare", "tiposiva", "codigiva", RF!tp1faccl, "N", Cad3)
    If Cad2 <> "" Then
        ...
        End If
    Else
        MsgBox "Error leyendo TIPO de IVA: " & RS!tp1faccl, vbExclamation
        RF.Close
        Exit Function
    End If
    ...
    If Not IsNull(RF!tp2faccl) Then
        Cad3 = "cuentarr"
        Cad2 = DevuelveDesdeBD("cuentare", "tiposiva", "codigiva", RF!tp2faccl, "N", Cad3)
        If Cad2 <> "" Then
            SQL = Mes & ",'" & Cad2 & "'," & DocConcAmp
            'Importes, atencion importes negativos
            ...
            End If
        Else
            MsgBox "Error leyendo TIPO de IVA2: " & RS!tp2faccl, vbExclamation
            RF.Close
            Exit Function
        End If
    ...
    If Not IsNull(RF!tp3faccl) Then
        Cad3 = "cuentarr"
        Cad2 = DevuelveDesdeBD("cuentare", "tiposiva", "codigiva", RF!tp3faccl, "N", Cad3)
        If Cad2 <> "" Then
            SQL = Mes & ",'" & Cad2 & "'," & DocConcAmp
            'Importes, atencion importes negativos
            ...
            End If
        Else
            MsgBox "Error leyendo TIPO de IVA 3: " & RS!tp3faccl, vbExclamation
            RF.Close
            Exit Function
        End If
...
'    End If
    ...
    While Not RF.EOF
        ...
        SQL = Mes & ",'" & RF!codtbase & "'," & DocConcAmp
    ...
    SQL = "UPDATE cabfact SET numdiari = " & DiarioFacturas & ", fechaent = '" & Fecha & "', numasien =" & NumAsiento
    SQL = SQL & " WHERE numserie='" & NUmSerie
    SQL = SQL & "' AND codfaccl= " & NumFac
    SQL = SQL & " AND anofaccl=" & NumDiari
    Conn.Execute SQL
...
'Marzo 2015
' Factura inversion de sujeto pasivo
' Por cada IVA entrará un linea mas , ya que ademas del soportado, pondremos el repercutido
'Dim InversionSujetoPasivo As Boolean    ver bajo feb2016
'Febrero 2016
'El tratamiento contable de las facturas intracomunitarias es el mismo
'Por lo tanto La variable que era InversionSujetoPasivo pasa a ser NoSumaIVA
Dim NoSumaIVA As Boolean
    ...
    'InversionSujetoPasivo = RF!extranje = 3        'Abril 2015
    ...
    A_Donde = "IVA 1"
    Cad3 = "cuentasr"
    Cad2 = DevuelveDesdeBD(ColumnaIVA, "tiposiva", "codigiva", RF!tp1facpr, "N", Cad3)
    If Cad2 <> "" Then
        SQL = Mes & ",'" & Cad2 & "'," & DocConcAmp
        ...
        If NoSumaIVA Then
            Cad3 = "cuentarr"
            Cad2 = DevuelveDesdeBD("cuentare", "tiposiva", "codigiva", RF!tp1facpr, "N", Cad3)
            
            Cad3 = Cad2 & "|" & Cad3 & "|"
            
            
            SQL = Mes & ",'" & RecuperaValor(Cad3, 1) & "'," & DocConcAmp
            Cad2 = CadenaImporte(False, RF!ti1facpr, Importe0)
            SQL = SQL & "," & Cad2 & ","
            SQL = SQL & "NULL,'" & RF!codmacta & "','FRAPRO',0)"
            If Not Importe0 Then
                Conn.Execute Cad & SQL
            ...
            If Not IsNull(RF!tr1facpr) Then
                ...
                'Importes, atencion importes negativos
                Cad2 = CadenaImporte(False, RF!tr1facpr, Importe0)
                SQL = SQL & "," & Cad2 & ","
                SQL = SQL & "NULL,'" & RF!codmacta & "','FRAPRO',0)"
                If Not Importe0 Then
                    Conn.Execute Cad & SQL
    ...
    Else
        MsgBox "Error leyendo TIPO de IVA: " & RS!tp1facpr, vbExclamation
        RF.Close
        Exit Function
    End If
    ...
    If Not IsNull(RF!tp2facpr) Then
        Cad3 = "cuentasr"
        Cad2 = DevuelveDesdeBD(ColumnaIVA, "tiposiva", "codigiva", RF!tp2facpr, "N", Cad3)
        If Cad2 <> "" Then
            SQL = Mes & ",'" & Cad2 & "'," & DocConcAmp
            'Importes, atencion importes negativos
            ...
            If NoSumaIVA Then
                Cad3 = "cuentarr"
                Cad2 = DevuelveDesdeBD("cuentare", "tiposiva", "codigiva", RF!tp2facpr, "N", Cad3)
                
                Cad3 = Cad2 & "|" & Cad3 & "|"
                
                
                SQL = Mes & ",'" & RecuperaValor(Cad3, 1) & "'," & DocConcAmp
                Cad2 = CadenaImporte(False, RF!ti2facpr, Importe0)
                SQL = SQL & "," & Cad2 & ","
                SQL = SQL & "NULL,'" & RF!codmacta & "','FRAPRO',0)"
                If Not Importe0 Then
                    Conn.Execute Cad & SQL
                ...
                If Not IsNull(RF!tr2facpr) Then
                     SQL = Mes & "," & RecuperaValor(Cad3, 1) & "," & DocConcAmp
                    'Importes, atencion importes negativos
                    Cad2 = CadenaImporte(False, RF!tr2facpr, Importe0)
                    SQL = SQL & "," & Cad2 & ","
                    SQL = SQL & "NULL,'" & RF!codmacta & "','FRAPRO',0)"
                    If Not Importe0 Then
                        Conn.Execute Cad & SQL
        ...
        Else
            MsgBox "Error leyendo TIPO de IVA2: " & RS!tp1facpr, vbExclamation
            RF.Close
            Exit Function
        End If
    ...
    If Not IsNull(RF!tp3facpr) Then
        Cad3 = "cuentasr"
        Cad2 = DevuelveDesdeBD(ColumnaIVA, "tiposiva", "codigiva", RF!tp3facpr, "N", Cad3)
        If Cad2 <> "" Then
            SQL = Mes & ",'" & Cad2 & "'," & DocConcAmp
            'Importes, atencion importes negativos
            ...
            If NoSumaIVA Then
                Cad3 = "cuentarr"
                Cad2 = DevuelveDesdeBD("cuentare", "tiposiva", "codigiva", RF!tp3facpr, "N", Cad3)
                
                Cad3 = Cad2 & "|" & Cad3 & "|"
                
                
                SQL = Mes & ",'" & RecuperaValor(Cad3, 1) & "'," & DocConcAmp
                Cad2 = CadenaImporte(False, RF!ti3facpr, Importe0)
                SQL = SQL & "," & Cad2 & ","
                SQL = SQL & "NULL,'" & RF!codmacta & "','FRAPRO',0)"
                If Not Importe0 Then
                    Conn.Execute Cad & SQL
                ...
                If Not IsNull(RF!tr3facpr) Then
                     SQL = Mes & "," & RecuperaValor(Cad3, 1) & "," & DocConcAmp
                    'Importes, atencion importes negativos
                    Cad2 = CadenaImporte(False, RF!tr3facpr, Importe0)
                    SQL = SQL & "," & Cad2 & ","
                    SQL = SQL & "NULL,'" & RF!codmacta & "','FRAPRO',0)"
                    If Not Importe0 Then
                        Conn.Execute Cad & SQL
        ...
        Else
            MsgBox "Error leyendo TIPO de IVA 3: " & RS!tp1facpr, vbExclamation
            RF.Close
            Exit Function
        End If
...
'    End If
    ...
    'Actualimos en factura, el nº de asiento
    SQL = "UPDATE cabfactprov SET numdiari = " & DiarioFacturas & ", fechaent = '" & Fecha & "', numasien =" & NumAsiento
    SQL = SQL & " WHERE  numregis = " & NumFac
    SQL = SQL & " AND anofacpr=" & NumDiari
    Conn.Execute SQL
...
'    SQL = SQL & "));"
    ...
    'sEGUNO INTENTO. Pero NO el definitivo. El de arriba estaba mal el SQL demasiado largo
    'Este no agrupa por cuentas
...
Private Function DesActualizaAsiento() As Boolean
    ...
    If Me.OpcionActualizar = 3 Then
        If AsientoExiste(False) Then Exit Function
    End If
    'Aqui bloquearemos
...
'    SQL = SQL & "));"
    ...
    While Not RL.EOF
        ...
        ImporteD = DBLet(RL!sD, "N")
        ImporteH = DBLet(RL!sH, "N")
        If Not CalcularSaldosDesactualizar Then
            RL.Close
            Exit Function
        End If
        'Sig
    ...
    'FALTA###  QUitar estos GROUPS BY
'    SQL = "SELECT Sum(hlinapu.timporteD) AS SD, Sum(hlinapu.timporteH) AS SH, hlinapu.codmacta,"
'    SQL = SQL & " hlinapu.fechaent, hlinapu.numdiari, hlinapu.numasien, hlinapu.codccost,cabccost.idsubcos"
'    SQL = SQL & " From hlinapu,cabccost WHERE hlinapu.codccost=cabccost.codccost"
'    SQL = SQL & " GROUP BY hlinapu.codmacta, hlinapu.fechaent, hlinapu.numdiari, hlinapu.numasien, hlinapu.codccost"
'    SQL = SQL & " HAVING (((hlinapu.numdiari)=" & NumDiari
'    SQL = SQL & ") AND ((hlinapu.fechaent)='" & Fecha & "'"
'    SQL = SQL & " ) AND ((hlinapu.numasien)=" & NumAsiento
'    SQL = SQL & ") AND ((hlinapu.codccost) Is Not Null));"
...
Private Function CalcularSaldosDesactualizar() As Boolean
    Dim I As Integer
    CalcularSaldosDesactualizar = False
    For I = vEmpresa.numnivel To 1 Step -1
        If Not CalcularSaldos1NivelDesactualizar(I) Then Exit Function
    Next I
    CalcularSaldosDesactualizar = True
End Function



Private Function CalcularSaldos1NivelDesactualizar(Nivel As Integer) As Boolean
    Dim ImpD As Currency
    Dim ImpH As Currency
    Dim TD As String
    Dim TH As String
    ...
    CalcularSaldos1NivelDesactualizar = False
    I = DigitosNivel(Nivel)
    If I < 0 Then Exit Function
    ...
    RS.Open SQL, Conn, adOpenForwardOnly, adLockOptimistic, adCmdText
    If RS.EOF Then
        'MsgBox "Error grave. No existe saldos para la cuenta: " & Cta
        'RS.Close
        'Exit Function
        'Esto no sabemos si es del todo correcto
        ImpD = 0
        ImpH = 0
    Else
        I = 1
        ImpD = DBLet(RS.Fields(0), "N")
        ImpH = DBLet(RS.Fields(1), "N")
    End If
    RS.Close
    'Quitamos el importe
    ImpD = ImpD - ImporteD    'Con respecto a ACTUALIZAR CAMBIA EL SIGNO
    ...
    If ImpD = 0 And ImpH = 0 Then
        'Si los importes son 0, borramos todos los datos
        SQL = "DELETE  FROM hsaldos"
    Else
        SQL = "UPDATE hsaldos SET Impmesde=" & TD & ", Impmesha = " & TH
    End If
    SQL = SQL & " WHERE Codmacta = '" & Cta & "' AND Anopsald = " & Anyo & " AND mespsald = " & Mes
    ...
    Conn.Execute SQL
    CalcularSaldos1NivelDesactualizar = True
End Function
...
'CCost = OtrosPCsContraContabiliad(False)
...
'    Do
'        SQL = RecuperaValor(CCost, CInt(INC))
'        If SQL <> "" Then
...
Private Sub HacerRecalculo()
Dim RS1 As Recordset
Dim N As Integer
Dim Co As Collection

On Error GoTo EHacerRecalculo

'Eliminamos de hsaldos donde se mayor que mes ejercicio
    Label11.Caption = "Eliminado hsaldos y hsaldos ana."
    Cuenta = Year(vParam.fechaini)
    SQL = "DELETE FROM hsaldos WHERE anopsald >" & Cuenta
    Conn.Execute SQL
    SQL = "DELETE FROM hsaldosanal WHERE anoccost >" & Cuenta
    Conn.Execute SQL
    
    Fecha = " mespsald >= " & Month(vParam.fechaini) & " AND anopsald = " & Cuenta
    SQL = "DELETE FROM hsaldos WHERE " & Fecha
    Conn.Execute SQL
    
    Fecha = " mesccost>=" & Month(vParam.fechaini) & " AND anoccost = " & Cuenta
    SQL = "DELETE FROM hsaldosanal WHERE " & Fecha
    Conn.Execute SQL
    
    
    '-----------------------------------------------------------
    'Para cada asiento del hco actualizamos los saldos
    ...
    For Mes = vEmpresa.numnivel To 1 Step -1
        
    
        
        Label11.Caption = "Nivel: " & Mes
        Label11.Refresh
        
        
        NumDiari = DigitosNivel(Mes)
        If NumDiari = vEmpresa.DigitosUltimoNivel Then
            SQL = "select codmacta "
        Else
            SQL = "Select substring(codmacta,1," & NumDiari & ")"
        End If
        SQL = SQL & " as cta,year(fechaent) as ano,month(fechaent) as mes,sum(timported)as debe,sum(timporteH)as haber from hlinapu "
        SQL = SQL & " WHERE fechaent>='" & Format(vParam.fechaini, FormatoFecha) & "' "
        SQL = SQL & " group by  cta,ano,mes"
            
        
        RS1.Open SQL, Conn, adOpenForwardOnly, adLockPessimistic, adCmdText
        Cuenta = ""
        SQL = "INSERT INTO hsaldos (codmacta, anopsald, mespsald, impmesde, impmesha) VALUES "
        While Not RS1.EOF
            
            Label11.Caption = RS1!Cta & " " & RS1!Ano & "   " & RS1!Mes
            Label11.Refresh
            
            If Cuenta <> "" Then Cuenta = Cuenta & ","
            Cuenta = Cuenta & "('" & RS1!Cta & "'," & RS1!Ano & "," & RS1!Mes & "," & TransformaComasPuntos(CStr(DBLet(RS1!Debe, "N"))) & "," & TransformaComasPuntos(CStr(DBLet(RS1!Haber, "N"))) & ")"
            
            If Len(Cuenta) > 5000 Then InsertandoEnRecalculodeSaldos
            
            RS1.MoveNext
        Wend
        RS1.Close
        If Cuenta <> "" Then InsertandoEnRecalculodeSaldos
    Next Mes
    ...
    If vParam.autocoste Then
        ...
        Label11.Caption = "Analitica."
        Label11.Refresh
        DoEvents
        SQL = "select codmacta as cta,year(fechaent) as ano,month(fechaent) as mes,sum(timported)as debe,sum(timporteH)as haber,codccost from hlinapu"
        SQL = SQL & " Where fechaent>='" & Format(vParam.fechaini, FormatoFecha) & "' AND Not (codccost Is Null)"
        SQL = SQL & " group by  cta,codccost,ano,mes"
        ...
        While Not RS1.EOF
            ...
            Label11.Caption = RS1!Cta & " " & RS1!Ano & "   " & RS1!Mes & "  " & RS1!codccost
            Label11.Refresh
            If Cuenta <> "" Then Cuenta = Cuenta & ","
            Cuenta = Cuenta & "('" & RS1!Cta & "'," & RS1!Ano & "," & RS1!Mes & "," & TransformaComasPuntos(CStr(DBLet(RS1!Debe, "N"))) & "," & TransformaComasPuntos(CStr(DBLet(RS1!Haber, "N"))) & ",'" & RS1!codccost & "')"
        ...
        'Cojere los centros que tienen reparto de base y lo efectuare
        Label11.Caption = "Analitica reparto bases"
        Label11.Refresh
        DoEvents
        SQL = "select hsaldosanal.codccost from hsaldosanal,cabccost where hsaldosanal.codccost=cabccost.codccost and idsubcos=1 group by codccost"
        Set Co = New Collection
        RS1.Open SQL, Conn, adOpenForwardOnly, adLockPessimistic, adCmdText
        If Not RS1.EOF Then
            
            'Ya tengo los CC que tienen repartp
            While Not RS1.EOF
                Co.Add CStr(RS1!codccost)
                RS1.MoveNext
            Wend
        ...
        If Co.Count > 0 Then
            ...
            While Co.Count <> 0
                Label11.Caption = "Ccost: " & Co.Item(Co.Count)
                Label11.Refresh
                DoEvents
            
            
                SQL = "select hsaldosanal.* from hsaldosanal where hsaldosanal.codccost = '" & Co.Item(Co.Count) & "'"
                RS1.Open SQL, Conn, adOpenForwardOnly, adLockOptimistic, adCmdText
                

                'Ya tengo los centros de reparto
                'Cojere todos los datos de hsaldos anal que tengan
                'Y efectuare su reparto para cada mes
                While Not RS1.EOF
                    ...
                    Anyo = RS1!anoccost
                    Mes = RS1!mesccost
                    Label11.Caption = Cuenta & " - " & CCost & " .   " & Mes & "/" & Anyo
                    Label11.Refresh
                    ImporteD = DBLet(RS1!debccost, "N")
                    ImporteH = DBLet(RS1!habccost, "N")

                    If Not HacerReparto(True) Then
                        MsgBox "Error grave generadndo datos anailitica", vbExclamation
                        RS1.Close
                        GoTo EHacerRecalculo
                    End If
                    RS1.MoveNext
                Wend
                RS1.Close
