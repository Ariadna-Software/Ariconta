...
Begin VB.Form frmInmov 
   ...
   Begin VB.Frame Frame0 
      ...
      Begin VB.ComboBox Combo1 
         Height          =   315
         ItemData        =   "frmInmov.frx":CE74
         Left            =   1920
         List            =   "frmInmov.frx":CE84
...
Private WithEvents frmE As frmEltoInmo
...
Private Sub cmdCalcula_Click()
    ...
    If B Then
        ...
        PreparaBloquear
            Conn.BeginTrans
            Cad = "Select * from sinmov where   sinmov.fecventa is null and sinmov.valoradq > sinmov.amortacu and situacio=1"
            'Fecha adq
...
Dim RC As String
    ...
    ParametrosContabiliza = ParametrosContabiliza & " FROM sconam,sinmov WHERE sconam.codconam=sinmov.conconam"
    RC = ""
...
Dim RC As String
    ...
    'La primera parte es comun para todos los informes
    ParametrosContabiliza = "SELECT codconam,nomconam,codinmov,nominmov,tipoamor,coeficie,"
    ParametrosContabiliza = ParametrosContabiliza & "codprove,fechaadq,valoradq,amortacu,fecventa,impventa,nomccost,cabccost.codccost "
    'ANtes
    'ParametrosContabiliza = ParametrosContabiliza & " FROM sconam,sinmov,cabccost WHERE sconam.codconam=sinmov.conconam and sinmov.codccost=cabccost.codccost"
    'AHora
    ParametrosContabiliza = ParametrosContabiliza & " FROM (sinmov INNER JOIN sconam ON sinmov.conconam = sconam.codconam) LEFT JOIN cabccost ON sinmov.codccost = cabccost.codccost"
    ...
    If txtCodCCost(0).Text <> "" Then
        ...
        Cad = Cad & "cabccost.codccost >= '" & txtCodCCost(0).Text & "'"
    End If
    If txtCodCCost(1).Text <> "" Then
        ...
        Cad = Cad & "cabccost.codccost <= '" & txtCodCCost(1).Text & "'"
    End If
    'Segundo parametros
    RC = "Centro= """ & RC & """|"
    AUX2 = AUX2 & RC
...
Dim RC As String
    ...
    If Text6(1).Text <> "" Then
        If Cad <> "" Then Cad = Cad & " AND "
        Cad = Cad & "sinmov.codinmov >= " & Text6(1).Text
    ...
    If Text6(2).Text <> "" Then
        If Cad <> "" Then
            ...
            TipoIva = TipoIva & "    "
        End If
        Cad = Cad & " sinmov.codinmov <= " & Text6(2).Text
    ...
    End If
    ParametrosContabiliza = "SELECT sconam.codconam, sconam.nomconam, sinmov.codinmov, sinmov.nominmov, sinmov.fechaadq,"
    ParametrosContabiliza = ParametrosContabiliza & "sinmov.valoradq, shisin.fechainm, shisin.imporinm, shisin.porcinm"
    ParametrosContabiliza = ParametrosContabiliza & " FROM sinmov,sconam ,shisin WHERE    sinmov.conconam = sconam.codconam"
    ParametrosContabiliza = ParametrosContabiliza & " and sinmov.codinmov = shisin.codinmov"
...
Dim RC As String
    RC = "tipoamor"
    Cad = DevuelveDesdeBD("ultfecha", "samort", "codigo", "1", "N", RC)
...
On Error GoTo ECargarDatos
    CargarDatos = False
    Set RS = New ADODB.Recordset
    Cad = "Select * from samort where codigo=1"
...
Private Sub Text6_LostFocus(Index As Integer)
    ...
    With Text6(Index)
        ...
        Cad = DevuelveDesdeBD("nominmov", "sinmov", "codinmov", .Text, "N", ParametrosContabiliza)
        If Cad = "" Then
...
Private Sub Text8_LostFocus(Index As Integer)
    ...
    If Index = 0 Then
        ...
        If Not IsNumeric(Text8(0).Text) Then
            ...
            I = 1
        Else
            Cad = DevuelveDesdeBD("nomforpa", "sforpa", "codforpa", Text8(0).Text, "N")
            I = 2
        End If
        If Cad = "" Then
...
On Error GoTo EInsertarModificar
    ...
    If Me.Tag = "1" Then
        'Modificar
        Cad = "UPDATE samort SET tipoamor= " & Combo1.ListIndex + 1
    ...
    Else
        'INSERTAR
        Cad = "INSERT INTO samort (codigo, tipoamor, intcont, ultfecha, condebes, conhaber, numdiari,codiva,preimpreso) VALUES (1,"
...
Private Sub txtCodCCost_LostFocus(Index As Integer)
    ...
    If txtCodCCost(Index).Text = "" Then
        ...
        Exit Sub
    End If
    Cad = DevuelveDesdeBD("nomccost", "cabccost", "codccost", txtCodCCost(Index).Text, "T")
    If Cad = "" Then
...
Private Sub txtConce_LostFocus(Index As Integer)
    ...
    End If
    If I = 0 Then
        Cad = DevuelveDesdeBD("nomconam", "sconam", "codconam", txtConce(Index).Text, "N")
        If Cad = "" Then
...
With txtCta(Index)
    ...
    If CuentaCorrectaUltimoNivel(ParametrosContabiliza, Cad) Then
        ...
        If Index = 3 Then
            Cad = DevuelveDesdeBD("codmacta", "ctabancaria", "codmacta", ParametrosContabiliza, "T")
            If Cad = "" Then
...
On Error GoTo EGen
    ...
    If Opcion <> 3 Then
        Cad = "UPDATE samort SET ultfecha= '" & Format(Cad, FormatoFecha)
        Cad = Cad & "' WHERE codigo=1"
        Conn.Execute Cad
        RS.Close
    Else
...
On Error GoTo ECar
    ...
    Cad = "Select * from sinmov where codinmov =" & Text6(0).Text & " for update"
    Set RS = New ADODB.Recordset
...
'   Es para baja
    If Option1(1).Value Then
        Cad = "UPDATE sinmov SET fecventa = '" & Format(Text4(0).Text, FormatoFecha)
        Cad = Cad & "', situacio =3  "
...
On Error GoTo EVentaElemento
    VentaElemento = False
        ...
        TipoIva = DevuelveDesdeBD("codiva", "samort", "codigo", "1", "N")
        If TipoIva = "" Then
        ...
        If Importe > 0 Then
            ...
            Aux = "UPDATE sinmov set amortacu=valoradq Where codinmov =" & Text6(0).Text
            Conn.Execute Aux
        ...
        If vEmpresa.TieneTesoreria Then
            ...
            Aux = DevuelveDesdeBD("tipforpa", "sforpa", "codforpa", Text8(0).Text, "N")
            I = Val(Aux)
...
Cad = DevuelveDesdeBD("situacio", "sinmov", "codinmov", Text6(0).Text, "N")
If Cad <> "" Then
...
Dim Veces As Integer
    ...
    Cad = Cad & " SELECT  " & vUsu.Codigo & ",codinmov,0,sum(imporinm) FROM  shisin WHERE "
    'las fechas
    ...
    'Nuevo Marzo 2010
    'Para los elementos que por algun motivo no tienen valor en shisin pero si que deberian salir
    ...
    '   Segunda. Eltos que no estan en shisin(por algun motivo) pero tienen que salir
    For Veces = 1 To 2
        ...
        Else
            ...
            'no esten en shisin pero si que si que puedan salir
            ...
            AUX2 = AUX2 & "' AND not codinmov IN (select distinct(codinmov) from shisin)"
            AUX2 = " SELECT " & vUsu.Codigo & ",codinmov,0,0 totalamor FROM sinmov " & AUX2
        ...
        Wend
        RT.Close
    Next Veces '** LO hago dos veces. La de antes y la de busqueda de eltos  que no tienene en shisin
...
Private Sub CargarDatosAmortizacion()
    ...
    UltAmor = "01/01/1901"
    Cad = DevuelveDesdeBD("ultfecha", "samort", "codigo", "1", "N")
...
Private Sub DeshacerUltimaAmortizacion()
    ...
    'Deshacemos en shisin y en sinmov. En los inmovilizados propiamente dicho
...
Dim F As Date
    ...
    Cad = "Select count(*) from shisin where fechainm = '" & Format(UltAmor, FormatoFecha) & "'"
    Cont = 0
    RS.Open Cad, Conn, adOpenForwardOnly, adLockPessimistic, adCmdText
    ...
    Cad = "select l.codinmov,imporinm,amortacu,valoradq,nominmov from shisin l,sinmov where l.codinmov=sinmov.codinmov "
    ...
    While Not RS.EOF
        ...
        'Creo SQL update
        Cad = "UPDATE sinmov set amortacu=" & TransformaComasPuntos(CStr(Importe))
        Cad = Cad & ", situacio= 1"
    ...
    'Borramos todos los datos de shisin con esta fecha
    Cad = "DELETE from shisin where fechainm = '" & Format(UltAmor, FormatoFecha) & "'"
    Conn.Execute Cad
    
    'ACtualizamos la fecha de ultamor
    '--------------------------------
    AUX2 = "tipoamor"
    Cad = DevuelveDesdeBD("intcont", "samort", "codigo", "1", "N", AUX2)
    Contabiliza = (Cad = 1)
    DivMes = Val(AUX2)
    Select Case DivMes
        Case 2
    ...
    Cad = "UPDATE samort set ultfecha='" & Cad & "'"
    Conn.Execute Cad
...
Private Function Datosok_Deshacer() As Boolean
    ...
    Cad = "select distinct(shisin.codinmov) from shisin, sinmov where shisin.codinmov=sinmov.codinmov and"
    ...
    Cad = "select distinct(shisin.codinmov) from shisin where  fechainm > '" & Format(UltAmor, FormatoFecha) & "'"
    RS.Open Cad, Conn, adOpenForwardOnly, adLockPessimistic, adCmdText
    Cont = 0
    While Not RS.EOF
        Cont = Cont + 1
